...
这是一个部署在 GitHub Pages 上的非官方歌曲资料索引站。网站只展示作品资料、版本资料、完整歌词和外部链接；授权音频通过详情页“下载”按钮选择音质下载，不提供站内播放器。
...
首页刊头立绘和页眉头像是可选的。将确认可公开展示的图片放入 `public/media/site/`，再在 `src/site.ts` 的 `BRAND_IMAGES` 填写相对于 `public/` 的路径即可启用；未配置时使用纯排版样式，不会发起图片请求。刊头建议透明背景 PNG 或 WebP、高度不低于 900px；头像建议正方形、不小于 128×128。
...
在管理页面中可以新建或编辑作品、版本、人员、标签选择、链接、歌词、封面和多个音质的授权音频。标签分组和标签定义仍手工维护 `public/content/tags.json`。保存后请点击“校验曲库”，再查看 `git diff`，确认无误后手动提交并推送。管理工具不会直接修改 `works-index.json`；公开站点构建时会重新生成它。
